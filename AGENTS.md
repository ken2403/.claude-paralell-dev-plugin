...
- **Dual-model final review, by default.** Final rounds use the same `dual-review.sh` parallel orchestrator as standalone review (`CA_DUAL_REVIEW=0` is the Claude-only opt-out; checkpoints stay Claude-only by design): a blind Claude `/ca:review-pr`, an offline Codex second-opinion leg (`codex-review.sh`, host-fetched PR data, explicit `$ca-second-opinion`, `codex exec --disable multi_agent --sandbox read-only --output-schema`, no network/gh inside Codex), and, when needed, a separate Claude `/ca:synthesize-review` call. The Codex child runs from an isolated temporary root and reads the target worktree by absolute path, so repository-specific review skills cannot hijack the bounded leg through implicit invocation. The current Codex artifact stays outside the worktree until blind Claude finishes. Codex findings are advisory untrusted claims; synthesis is the only place they can affect the final `ca_claude_review.v1` verdict. Clean full-coverage Codex output with zero findings skips synthesis (`clean_no_synthesis`); unavailable/invalid/timeout Codex degrades visibly to Claude-only with a `.ca/runs/<id>/review-round-N.meta.json` sidecar. Checkpoints are intentionally Claude-only progress gates, not dual review: they cannot promote the PR, they run once per milestone, and in-loop the Codex leg is the implementer reviewing itself — measured twice at zero findings on its own work — so the heavier check is spent where it gates, on the final round.
...
- **Both plugins are required.** The Codex plugin supplies `$ca-implement-plan` and the internal explicit-only `$ca-second-opinion`; the Claude plugin supplies `/ca:review-pr` and `/ca:synthesize-review`. `claude-review.sh` needs the Claude plugin resolvable (installed or through `CA_CLAUDE_PLUGIN_DIR`), and a real Codex leg needs the Codex plugin installed so the required skill resolves. Installing only one side cannot produce the full dual loop.
...
- **Blind isolation is two-way.** `dual-review.sh` keeps BOTH legs' artifacts in a private dir until BOTH finish: the Codex leg reviews with read access to the worktree, so publishing the blind verdict into `.ca/runs` early would make the "second opinion" an echo. The Codex prompt also tells it not to read `.ca/runs` / `.ca/reviews` (the mirror of the blind prompt's instruction), covering earlier rounds' artifacts.

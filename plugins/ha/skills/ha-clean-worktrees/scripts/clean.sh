#!/usr/bin/env bash
# ==============================================================================
# Self-contained worktree cleanup script for the Codex HA plugin.
#
...
#     (e.g. a finished Claude/Codex session's "pid N" lock) is unlocked and removed;
...
for item in "${MERGED_JOBS[@]}"; do
  ...
  # lock whose holder is DEAD (e.g. "codex session ... (pid N ...)" from a
  ...
  if [ -n "$lock_line" ]; then
    ...
    # must match the exact "claude|codex session ... (pid N ...)" shape, ANCHORED at
    ...
    lock_pid="$(printf '%s' "$lock_reason" | sed -E -n 's/^(claude|codex) session .*\(pid ([0-9]+)[^)]*\).*$/\2/p')"
    if [ -z "$lock_pid" ]; then
      echo "  SKIP: locked (${lock_reason:-no reason}) — not a recognized session lock; 'git worktree unlock' manually if intended"
      FAILED_JOBS+=("$name")
      continue
    fi
    ...
    echo "  Stale session lock (holder pid $lock_pid is gone) — unlocking"
